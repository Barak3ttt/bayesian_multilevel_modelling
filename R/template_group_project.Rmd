...
# 1. Install Kaggle API if you haven’t yet
if (system("which kaggle", ignore.stderr = TRUE) != 0) {
  system("pip install kaggle")
}

# 2. Load your credentials from ~/.kaggle/kaggle.json
library(jsonlite)
creds <- fromJSON("~/.kaggle/kaggle.json")
Sys.setenv(
  KAGGLE_USERNAME = creds$barakazor,
  KAGGLE_KEY      = creds$k22891584
)

# 3. Create a data/ directory (if needed) and download & unzip
if (!dir.exists("data")) dir.create("data")
system("kaggle datasets download -d prasertk/inflation-interest-and-unemployment-rate -p data --unzip")

# 4. Read the CSV into R
df_raw <- read.csv("data/inflation-interest-and-unemployment-rate.csv",
