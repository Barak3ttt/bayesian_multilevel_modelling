...
# Define priors for the cumulative model with logit link and random intercepts (debugging)
logit_intercept_priors_debug <- c(
...
# Fit a standalone brms model (cumulative with LOGIT link - debug attempt 3: changed link)
print("Fitting a cumulative brm model with LOGIT link and random intercepts (debugging attempt 3)...")
fit_cumulative_logit_intercepts <- brm(
  formula = bf(
  ...
  prior   = logit_intercept_priors_debug,
  iter    = 2000,
  warmup  = 1000,
...
print("Cumulative LOGIT brm model fitting complete (debugging attempt 3).")
print(summary(fit_cumulative_logit_intercepts))
...
**Model 1: Cumulative Ordinal Regression with Random Intercepts and LOGIT Link (Debugging Version)**

This model, named `fit_cumulative_logit_intercepts` in the R code, aims to predict the ordinal credit `Rating` based on several financial predictors.

*   **Dependent Variable**: `Rating`, treated as an ordered categorical variable.
*   **Likelihood and Link Function**: The model uses a `cumulative` likelihood with a `logit` (logistic) link function.
    ...
    *   The `logit` link is a common choice, modeling the log-odds of falling into a category or lower. It assumes a symmetric logistic distribution for the latent variable.
*   **Fixed Effects Structure**: The model includes the following predictors as main effects:
    *   `Current.Ratio`
    *   `Debt.Equity.Ratio`
    *   `Long.term.Debt...Capital`
    *   `EBITDA.Margin`
    *   `Operating.Cash.Flow.Per.Share`
    The interaction term between `Current.Ratio` and `Debt.Equity.Ratio` has been removed for this debugging attempt.
...
*   **Priors**:
    *   Fixed Effects (`class = b`): `normal(0, 1)` - This is a more conservative prior for the fixed effects compared to the original model.
    *   Standard Deviation of Random Intercepts (`class = sd` for `Sector`): `student_t(3, 0, 1)` - A more conservative prior for the standard deviation of the random intercepts.
    *   Cutpoints/Thresholds (`class = Intercept`): `normal(0, 1)` - A more conservative prior for the cutpoints compared to the original model.
*   **Assumptions**:
...
While our primary model (`fit_cumulative_logit_intercepts`) predicts ordinal ratings, we can derive binary classification metrics by defining a threshold to group these ratings. For this exercise, we will classify ratings as either "Investment Grade" (AAA down to BBB-) or "Speculative Grade" (BB+ down to D).
...
# Ensure the brms model object is available
if (!exists("fit_cumulative_logit_intercepts")) {
  stop("Model object 'fit_cumulative_logit_intercepts' not found. Please ensure the model fitting chunk has been run.")
}
...
# Define rating levels and investment grade threshold (full set for reference)
rating_levels_full_definition <- c("AAA","AA+","AA","AA-","A+","A","A-",
                                 "BBB+","BBB","BBB-","BB+","BB","BB-",
                                 "B+","B","B-","CCC","CC","C","D")
investment_grade_ratings <- c("AAA","AA+","AA","AA-","A+","A","A-", "BBB+","BBB","BBB-")

# Get the actual levels observed in the response variable during model training
observed_training_levels <- as.character(unique(fit_cumulative_logit_intercepts$data$Rating))

# Prepare a copy of test_rating for prediction
test_rating_for_pred <- test_rating

# Crucial step: Re-factor the Rating column in the newdata to match ONLY the levels seen in training data's response.
# Values in test_rating_for_pred$Rating not in observed_training_levels will become NA.
test_rating_for_pred$Rating <- factor(test_rating_for_pred$Rating, 
                                      levels = observed_training_levels, 
                                      ordered = TRUE)

# Identify rows that are still valid for prediction (Rating is not NA after re-factoring)
valid_pred_rows <- !is.na(test_rating_for_pred$Rating)

if(sum(valid_pred_rows) == 0){
    stop("No observations in test_rating have response levels matching the training data after re-factoring. Cannot proceed.")
}

# 1. Predict probabilities for each category on the valid subset of the test set
pred_probs_ordinal <- posterior_epred(fit_cumulative_logit_intercepts, 
                                      newdata = test_rating_for_pred[valid_pred_rows, ], 
                                      ndraws = 500)
print(paste("Dimensions of pred_probs_ordinal:", paste(dim(pred_probs_ordinal), collapse = ", ")))

# Get the number of categories from the 3rd dimension of the output array
num_categories_predicted <- dim(pred_probs_ordinal)[3]

if(num_categories_predicted == length(observed_training_levels)){
    # Correctly calculate mean probabilities: apply mean over the 1st dimension (draws)
    # Results in a matrix of [observations, categories]
    mean_pred_probs_ordinal <- apply(pred_probs_ordinal, c(2, 3), mean)
    colnames(mean_pred_probs_ordinal) <- observed_training_levels
} else {
    stop(paste("Mismatch between number of predicted categories (", num_categories_predicted,
               ") and length of observed training levels (", length(observed_training_levels), ")."))
}

# 2. Calculate the total probability of being investment grade for each valid observation
# Only consider investment grade ratings that were actually present in training data columns
relevant_investment_grade_ratings <- intersect(colnames(mean_pred_probs_ordinal), investment_grade_ratings)
prob_investment_grade <- rowSums(mean_pred_probs_ordinal[, relevant_investment_grade_ratings, drop = FALSE])
...
# 4. Determine true binary labels from the *valid* subset of original test_rating
# Use the original test_rating here for true labels, but only for rows that were valid for prediction
original_valid_ratings <- test_rating$Rating[valid_pred_rows]
true_binary_labels <- ifelse(original_valid_ratings %in% investment_grade_ratings, "Investment", "Speculative")
